...
function get20s(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    let test = array[i].years.split(" ");
    if (Number(test[0]) >= 1900 && Number(test[2]) <= 2000) {
...
function artistByCountry(array, nationalityType) {
  let nationalArr = [];
  for (let i = 0; i < array.length; i++) {
    if (array[i].nationality === nationalityType) {
      nationalArr.push(array[i].name);
    }
  }
  return nationalArr;
}
